...
namespace Authenticator.Repository.Port
{
    public class PortfolioRepo:IPortfolioRepo
    {
        ...
        private async Task<Portfolio> CreatePortfolio(int StockId)
        {
            var Existing = _db.PortfolioTable.Any(x => x.StockId == StockId);
            if (Existing)
            {
                throw new DuplicateNameException("Portfolio already exists");
            }
        ...
        public async Task<HttpStatusResult> PortfolioCreationValidation(int StockID,string Id)
        {
            try
            {
                var portFolio = await CreatePortfolio(StockID);
                if (portFolio == null)
                {
