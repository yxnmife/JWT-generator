...
namespace Authenticator.Repository.Account
{
...

    {
        ...
        public async Task<Accounts> Create(CreateAccountDTO create)
        {
           ...
           var exists= _db.AccountsTable.Any(a=>a.EmailAddress==create.EmailAddress && a.Username==create.Username);
            if (exists)
            {
                return null;
            }
      ...
      public async Task<HttpStatusResult> LoginValidation(string token)
        {
